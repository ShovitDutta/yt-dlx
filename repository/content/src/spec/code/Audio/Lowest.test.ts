...
console.clear();
dotenv.config();
function runAudioLowest(options: any): Promise<void> {
    return new Promise((resolve, reject) => {
        const stream = YouTubeDLX.Audio.Lowest(options);
        let handled = false;
        stream
            .on("data", data => {
...
(async () => {
    ...
    for (const testCase of testCases) {
        try {
            console.log(colors.bold.blue("@info"), `AudioLowest: (${testCase.label}): Running with options`, testCase.options);
            await runAudioLowest(testCase.options);
        } catch (error) {
