...
/**
 * @shortdesc Downloads or streams the lowest quality video from YouTube videos/queries with options for filters and output.
 *
 * @description This function allows fetching the lowest available quality video stream from a YouTube video identified by a query or URL.
 ...
 * - If `metadata` is `true`, it returns a Promise resolving to `{ metadata: object }` containing detailed video information including the lowest quality video format data (`BestVideoLow`, `VideoLowHDR`, `ManifestLow`) and a suggested filename.
 ...
 *
 * FFmpeg is used internally to process the video stream and set the output format to 'mp4'.
 *
 ...
 *
 * @example
 * // 1. Running Basic Download Example (fetches lowest quality video, saves in current directory)
 * try {
 ...
 *
 * @example
 * // 2. Running Download with Output and Filter Example (Grayscale)
 * try {
 ...
 *
 * @example
 * // 3. Running Download with All Options Example (Invert filter, Tor, Verbose, Progress)
 * try {
 ...
 *
 * @example
 * // 6. Running Basic Stream Example (fetches lowest quality video, pipes to a local file)
 ...
 *
 * @example
 * // 7. Running Stream with Filter Example (Flip Horizontal, pipes to a local file)
 * import { createWriteStream } from "fs";
 * try {
 ...
 *
 * @example
 * // 8. Running Stream with All Options Example (Rotate 90, Tor, Verbose, Progress, pipes to a local file)
 * import { createWriteStream } from "fs";
 * try {
 ...
 * try {
 * await VideoLowest({ query: "your search query or url", filter: "nonexistentvideofilter" as any }); // Using 'as any' to simulate invalid enum value
 * console.log("This should not be reached - Zod Validation Error Example.");
 * } catch (error) {
