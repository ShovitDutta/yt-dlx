...
/**
 * @shortdesc Downloads or streams the lowest quality video stream from YouTube videos/queries with video filters.
 *
 * @description This function fetches the lowest available quality video stream from a YouTube video identified by a query or URL.
 * It uses an internal engine (`Tuber`) to retrieve video information and the URL for the lowest quality video format,
 ...
 * The function does NOT include an audio stream in the output by default.
 * The function also supports fetching only the video's metadata, including information about the lowest quality video formats available.
 *
 ...
 * - If `metadata` is `true`, it returns a Promise resolving to `{ metadata: object }` containing detailed video information including data for the lowest quality video formats (`BestVideoLow`, `VideoLowHDR`, `ManifestLow`) and a suggested filename.
 ...
 *
 * FFmpeg is used internally to process the lowest quality video stream, apply video filters if specified, and output in 'mp4' format. Audio is NOT included in the output by this function.
 *
 ...
 * - Throws an `Error` if the lowest quality video URL (`BestVideoLow?.url`) is not found in the fetched data.
 ...
 *
 * @example
 * // 1. Running Basic Download Example (fetches lowest quality video, saves in current directory as MP4)
 * try {
 ...
 * try {
 * const result = await VideoLowest({ query: "your search query or url", output: "./full_downloads", useTor: false, verbose: true, filter: "invert", showProgress: true });
 ...
 * try {
 * const result = await VideoLowest({ query: "your search query or url", metadata: true, useTor: false, verbose: true });
 ...
 *
 * @example
 * // 6. Running Basic Stream Example (fetches lowest quality video, streams as MP4)
 ...
 *
 * @example
 * // 7. Running Stream with Video Filter Example (Applies flip filter)
 * import { createWriteStream } from "fs";
 * try {
 ...
 * try {
 * await VideoLowest({} as any); // Using 'as any' to simulate missing required parameter
 ...
 * try {
 * await VideoLowest({ query: "your search query or url", filter: "bassboost" as any }); // Using 'as any' to simulate invalid enum value
 * console.log("This should not be reached - Zod Validation Error Example.");
 * } catch (error) {
 ...
 * // try {
 * //    await VideoLowest({ query: "query-that-breaks-tuber" });
 * // } catch (error) {
 ...
 * //    await VideoLowest({ query: "your search query or url" });
 * // } catch (error) {
 ...
 * //    await VideoLowest({ query: "some video with format issues" });
 * // } catch (error) {
 ...
 * //    await VideoLowest({ query: "query-causing-ffmpeg-error" });
 * // } catch (error) {
