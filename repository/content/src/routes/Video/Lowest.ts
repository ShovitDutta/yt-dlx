...
// Define the Zod schema for input validation for VideoLowest
...
var ZodSchema = z.object({
    ...
    filter: z // Optional video filter (same enum as other video functions)
 ...
 *
 * @example
 * // 1. Download the lowest quality video using async/await syntax with try...catch
 * const query = "some video query";
 * try {
 * const outputPath = await YouTubeDLX.Video.Lowest({ query });
 ...
 *
 * @example
 * // 2. Stream the lowest quality video with verbose logging using async/await
 * const query = "another video query";
 * try {
 * const streamInfo = await YouTubeDLX.Video.Lowest({ query, stream: true, verbose: true });
 ...
 *
 * @example
 * // 3. Fetch only metadata for the lowest quality video using async/await
 * const query = "metadata test";
 * try {
 * const metadata = await YouTubeDLX.Video.Lowest({ query, metadata: true });
 * console.log("Metadata:", metadata);
 * console.log("Lowest video format details:", metadata.BestVideoLow);
 * } catch (error) {
 * console.error("Error fetching metadata:", error);
 * }
 *
 * @example
 * // 4. Download lowest quality video with a filter and custom output directory using async/await
 * const query = "video with filter";
 * try {
 * const outputPath = await YouTubeDLX.Video.Lowest({ query, output: "./filtered_videos_out", filter: "grayscale" });
...
export default async function VideoLowest({ query, stream, verbose, output, metadata, useTor, filter }: z.infer<typeof ZodSchema>): Promise<VideoLowestResult> {
    ...
    try {
        ...
        const lqVideoUrl = engineData.BestVideoLow?.url; // Use BestVideoLow for lowest video URL
        ...
        // Add the lowest quality video stream URL as the input to FFmpeg.
