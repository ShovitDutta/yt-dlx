...
// Define the Zod schema for input validation for VideoCustom
...
type MetadataResult = {
    ...
    VideoLowF: any; // Lowest quality video format details
    VideoHighF: any; // Highest quality video format details
...
type VideoCustomResult = MetadataResult | StreamResult | DownloadResult;

/**
 * @shortdesc Downloads, streams, or fetches metadata for video from YouTube with custom options using async/await instead of events.
 *
 * @description This function allows you to download, stream, or fetch metadata for video from YouTube based on a search query or video URL and a specified resolution using async/await.
 * It provides extensive customization options, including specifying the video resolution, applying various video filters, saving the output to a specified directory, using Tor for anonymity, enabling verbose logging, streaming the output, or simply fetching the metadata without downloading.
 *
 ...
 *
 * @returns {Promise<VideoCustomResult>} A Promise that resolves based on the operation mode:
    ...
    try {
        ...
        if (metadata) {
            ...
            return {
                ...
                VideoLowF: engineData.VideoLowF, // Include video format details from engineData
        ...
        // Add the video stream URL as the input to FFmpeg.
        instance.addInput(vdata.url.toString()); // Convert URL to string, as per original code's .toString() on vdata.url
