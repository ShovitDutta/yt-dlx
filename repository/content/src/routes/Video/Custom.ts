...
/**
 * @shortdesc Downloads or streams a specific video resolution from YouTube videos/queries with options for filters and output.
 *
 * @description This function allows fetching a YouTube video at a specified resolution, identified by a query or URL.
 * It uses an internal engine (`Tuber`) to retrieve video information and available video formats,
 * and then utilizes FFmpeg to process (optionally apply filters) and output the selected video stream.
 ...
 *
 * It requires a valid search query or URL to identify the YouTube video and a specific resolution.
 ...
 * - If `metadata` is `true`, it returns a Promise resolving to `{ metadata: object }` containing detailed video information including various format data (`VideoLowF`, `VideoHighF`, `VideoLowHDR`, `VideoHighHDR`, `ManifestLow`, `ManifestHigh`) and a suggested filename.
 ...
 *
 * FFmpeg is used internally, setting the output format to 'mp4'. If no filter is applied, it attempts to stream/download using `-c copy` for minimal processing.
 *
 ...
 *
 * @throws {Error}
 * - Throws a `ZodError` if the input options fail schema validation (e.g., missing required `query` or `resolution`, invalid enum value for `resolution` or `filter`).
 ...
 * - Throws an `Error` if no video data is found in the fetched manifest for the specified `resolution`.
 ...
 *
 * @example
 * // 1. Running Basic Download Example (saves 720p video in current directory)
 * try {
 ...
 *
 * @example
 * // 6. Running Basic Stream Example (streams 480p video, pipes to a local file)
 ...
 *
 * @example
 * // 13. Running Zod Validation Error Example (Invalid Resolution - will throw ZodError)
 * try {
 ...
 *
 * @example
 * // 14. Running Zod Validation Error Example (Invalid Filter - will throw ZodError)
 * try {
 * await VideoCustom({ query: "your search query or url", resolution: "720p", filter: "nonexistentfilter" as any }); // Using 'as any' to simulate invalid enum value
 * console.log("This should not be reached - Zod Validation Error Example.");
 * } catch (error) {
 ...
 * // This might occur if the internal engine's manifest doesn't contain data for the requested resolution.
 * // try {
 * //    // Use a query/resolution combination known to fail (e.g., a very old video at a high resolution)
 * //    await VideoCustom({ query: "some video", resolution: "8640p" }); // Example, might not always fail
 * // } catch (error) {
 ...
 * // This is similar to the above but might occur if the manifest entry exists but lacks a URL.
 * // try {
 * //    // Use a query/resolution combination known to cause this
