...
/**
 * @shortdesc Downloads or streams the highest quality video from YouTube videos/queries with options for video filters and output.
 *
 * @description This function fetches the highest available quality video stream from a YouTube video identified by a query or URL.
 * It uses an internal engine (`Tuber`) to retrieve video information and the URL for the best video format,
 * and then utilizes FFmpeg to process (optionally apply video filters) and output the selected video.
 * The output can be either a downloadable file saved locally or a readable stream.
 * The function also supports fetching only the video's metadata, including information about the highest quality video formats available.
 *
 ...
 * Note that this function primarily focuses on the video stream and does not explicitly include an audio stream input in the FFmpeg command. The resulting file/stream may contain video only, or may include audio if FFmpeg implicitly handles combining the best available audio stream from the source URL or manifest, depending on the input format.
 *
 ...
 * - **Filter:** An optional string specifying a video filter to apply using FFmpeg. Available filters include "invert", "rotate90", "rotate270", "grayscale", "rotate180", "flipVertical", "flipHorizontal". If no filter is specified, the video stream is copied without re-encoding if possible (using `-c copy`).
 * - **UseTor:** An optional boolean flag to route requests through Tor. Defaults to `false`.
 * - **Verbose:** An optional boolean value that, if true, enables detailed console logging during the fetch and processing stages. Defaults to `false`.
 * - **Metadata:** An optional boolean flag. If set to `true`, the function will only fetch and return the video metadata without performing any download or streaming. Cannot be used with `stream`, `output`, `filter`, or `showProgress`. Defaults to `false`. The returned metadata includes details about available high-quality video formats (`VideoHighF`, `VideoHighHDR`, `ManifestHigh`).
 ...
 * - If `metadata` is `true`, it returns a Promise resolving to `{ metadata: object }` containing detailed video information including data for the highest quality video formats and a suggested filename.
 ...
 *
 * FFmpeg is used internally to process and output the video stream in 'mp4' format, applying video filters if specified.
 *
 ...
 * - Throws an `Error` if the highest quality video URL (`VideoHighF?.url`) is not found in the fetched data.
 ...
 *
 * @example
 * // 1. Running Basic Download Example (fetches highest quality video, saves in current directory as MP4)
 * try {
 * const result = await VideoHighest({ query: "your search query or url" });
 ...
 * try {
 * const result = await VideoHighest({ query: "your search query or url", output: "./custom_downloads_vhigh", filter: "grayscale" });
 ...
 * try {
 * const result = await VideoHighest({ query: "your search query or url", output: "./full_downloads_vhigh", useTor: false, verbose: true, filter: "invert", showProgress: true });
 ...
 * try {
 * const result = await VideoHighest({ query: "your search query or url", metadata: true });
 ...
 * try {
 * const result = await VideoHighest({ query: "your search query or url", metadata: true, useTor: false, verbose: true });
 ...
 *
 * @example
 * // 6. Running Basic Stream Example (fetches highest quality video, streams as MP4)
 * import { createWriteStream } from "fs";
 * try {
 * const result = await VideoHighest({ query: "your search query or url", stream: true });
 * if ("stream" in result && result.stream) {
 * console.log("Basic Streaming started. Piping to basic_stream_vhigh.mp4...");
 ...
 *
 * @example
 * // 7. Running Stream with Video Filter Example (Applies rotation filter)
 * import { createWriteStream } from "fs";
 * try {
 * const result = await VideoHighest({ query: "your search query or url", stream: true, filter: "rotate90" });
 * if ("stream" in result && result.stream) {
 * console.log("Stream with Filter started. Piping to filtered_stream_vhigh.mp4...");
 ...
 * if ("stream" in result && result.stream) {
 * console.log("\nStream with All Options started. Piping to full_stream_vhigh.mp4...");
 ...
 * try {
 * await VideoHighest({ query: "your search query or url", metadata: true, output: "./should_fail_dir" });
 ...
 * try {
 * await VideoHighest({ query: "your search query or url", stream: true, output: "./should_fail_dir" });
 * console.log("This should not be reached - Invalid Options Example.");
 * } catch (error) {
 ...
 * try {
 * await VideoHighest({} as any); // Using 'as any' to simulate missing required parameter
 ...
 * try {
 * await VideoHighest({ query: "your search query or url", filter: "bassboost" as any }); // Using 'as any' to simulate invalid enum value
 * console.log("This should not be reached - Zod Validation Error Example.");
 * } catch (error) {
 ...
 * // try {
 * //    await VideoHighest({ query: "query-that-breaks-tuber" });
 * // } catch (error) {
 ...
 * //    await VideoHighest({ query: "your search query or url" });
 * // } catch (error) {
 * //    console.error("Expected Error (FFmpeg not found):", error instanceof Error ? error.message : error);
 * // }
 *
 * @example
 * // 15. Example of Highest Quality Video URL not found
 * // This might occur for certain videos if the internal engine cannot find the expected format.
 * // try {
 * //    // Use a query for a video known to have issues with highest quality video detection
 * //    await VideoHighest({ query: "some video with format issues" });
 * // } catch (error) {
 * //    console.error("Expected Error (Highest Quality Video URL not found):", error instanceof Error ? error.message : error);
 * // }
 *
 * @example
 ...
 * //    await VideoHighest({ query: "query-causing-ffmpeg-error" });
 * // } catch (error) {
...
export default async function VideoHighest({
    ...
    verbose,
    showProgress,
}: VideoHighestOptions): Promise<{ metadata: object } | { outputPath: string } | { stream: Readable }> {
    try {
