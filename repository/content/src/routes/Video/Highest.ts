...
/**
 * @shortdesc Downloads or streams the highest quality video from YouTube videos/queries with options for filters and output.
 *
 * @description This function allows fetching the highest available quality video from a YouTube video identified by a query or URL.
 ...
 * - If `metadata` is `true`, it returns a Promise resolving to `{ metadata: object }` containing detailed video information including the highest quality video format data (`VideoHighF`, `VideoHighHDR`, `ManifestHigh`) and a suggested filename.
 ...
 *
 * FFmpeg is used internally, setting the output format to 'mp4'. If no filter is applied, the video and audio streams are copied without re-encoding (`-c copy`). If a filter is applied, FFmpeg will re-encode the video.
 *
 ...
 * @param {"invert" | "rotate90" | "rotate270" | "grayscale" | "rotate180" | "flipVertical" | "flipHorizontal"} [options.filter] - Apply a video filter using FFmpeg.
 *
 ...
 *
 * @example
 * // 1. Running Basic Download Example (fetches highest quality video, saves in current directory)
 * try {
 ...
 * try {
 * const result = await VideoHighest({ query: "your search query or url", output: "./custom_downloads", filter: "grayscale" });
 ...
 * try {
 * const result = await VideoHighest({ query: "your search query or url", output: "./full_downloads", useTor: true, verbose: true, filter: "invert", showProgress: true });
 ...
 * try {
 * const result = await VideoHighest({ query: "your search query or url", metadata: true, useTor: true, verbose: true });
 ...
 *
 * @example
 * // 6. Running Basic Stream Example (fetches highest quality video, pipes to a local file)
 ...
 * import { createWriteStream } from "fs";
 * try {
 * const result = await VideoHighest({ query: "your search query or url", stream: true, useTor: true, verbose: true, filter: "rotate180", showProgress: true });
 * if ("stream" in result && result.stream) {
 ...
 * try {
 * await VideoHighest({ query: "your search query or url", filter: "nonexistentfilter" as any }); // Using 'as any' to simulate invalid enum value
 * console.log("This should not be reached - Zod Validation Error Example.");
 * } catch (error) {
...
(async () => {
    ...
    try {
        ...
        await VideoHighest({ query: "your search query or url", filter: "nonexistentfilter" as any });
        console.log("This should not be reached - Zod Validation Error Example.");
    } catch (error) {
        console.error("Expected Zod Error (Invalid Filter):", error instanceof Error ? error.message : error);
    }
