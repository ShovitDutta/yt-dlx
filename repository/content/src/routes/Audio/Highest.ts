...
// Define the Zod schema for input validation for AudioHighest
...
// Define types for the possible return values based on flags (similar to AudioCustom)
type MetadataResult = {
    metaData: any;
    AudioHighF: any; // Details for the highest quality audio format found by the engine
    AudioHighDRC: any; // Details for the highest quality audio with DRC found by the engine
 ...
 * // Use streamInfo.ffmpeg instance for piping the audio stream, e.g., streamInfo.ffmpeg.pipe(myWritableStream);
 ...
 *
 * @example
 * // 4. Download highest quality audio with a filter and custom output directory using async/await
 * const query = "video with filter";
 * try {
 * const outputPath = await YouTubeDLX.Audio.Highest({ query, output: "./filtered_audio_out", filter: "bassboost" });
...
export default async function AudioHighest({ query, output, useTor, stream, filter, metadata, verbose }: z.infer<typeof ZodSchema>): Promise<AudioHighestResult> {
    ...
    try {
        ...
        // Await the asynchronous call to the Agent (formerly Tuber) to get engine data.
        ...
        if (metadata) {
            ...
            return {
                ...
                AudioHighF: engineData.AudioHighF, // Include the highest audio format details
                AudioHighDRC: engineData.AudioHighDRC, // Include highest audio with DRC details
        ...
        // Add the URL of the highest quality audio stream as an input to FFmpeg.
        ...
        // Add the thumbnail URL as another input to FFmpeg.
        ...
        // Set the output destination for FFmpeg. If not streaming, it's the file path.
        // If streaming, the output stream is typically piped or handled differently by fluent-ffmpeg.
        if (!stream) {
            instance.output(outputPath);
        }
        // In streaming mode, the 'stream' event in the original code emitted the `outputPath` and `instance`.
        // The `outputPath` might be used as a reference name for the stream.
        ...
        if (stream) {
            ...
            const streamReadyPromise = new Promise<StreamResult>((resolve, reject) => {
                ...
                instance.on("start", commandLine => {
                    ...
                    // Resolve the promise with the stream information as soon as FFmpeg starts.
            ...
            // Await the promise that indicates the stream is ready (FFmpeg process has started).
            ...
            const downloadCompletePromise = new Promise<DownloadResult>((resolve, reject) => {
                ...
                // Listen for the FFmpeg 'end' event (successful completion).
                instance.on("end", () => {
        ...
        // Note: FFmpeg errors caught by the listeners attached to the instance are handled by
        // rejecting the specific promise being awaited (streamReadyPromise or downloadCompletePromise),
        // and these rejections will be caught by this outer try/catch block as exceptions.
    } finally {
        // This block executes after the try block successfully returns or the catch block throws an error.
