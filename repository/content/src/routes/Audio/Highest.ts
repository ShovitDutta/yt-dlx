...
describe("AudioHighest", () => {
    ...
    it("should throw error for metadata with output", async () => {
        await expect(AudioHighest({ query: "test query", metadata: true, output: "./should_fail_dir" })).rejects.toThrowError(/metadata.*cannot be used with.*output/);
    });
    it("should throw error for stream with output", async () => {
        await expect(AudioHighest({ query: "test query", stream: true, output: "./should_fail_dir" })).rejects.toThrowError(/stream.*cannot be used with.*output/);
    });
    it("should throw Zod error for missing query", async () => {
        await expect(AudioHighest({} as any)).rejects.toThrowError(/query.*Required/);
    });
    it("should throw Zod error for invalid filter", async () => {
        await expect(AudioHighest({ query: "test query", filter: "nonexistentfilter" as any })).rejects.toThrowError(/filter.*invalid enum value/);
    });
});
