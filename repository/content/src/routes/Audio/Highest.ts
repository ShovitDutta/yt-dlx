...
/**
 * @shortdesc Downloads or streams the highest quality audio from YouTube videos/queries with options for filters and output.
 *
 * @description This function allows fetching the highest available quality audio from a YouTube video identified by a query or URL.
 * It uses an internal engine (`Tuber`) to retrieve video information and the URL for the best audio format,
 ...
 * - If `metadata` is `true`, it returns a Promise resolving to `{ metadata: object }` containing detailed video information including the highest quality audio format data (`AudioHighF`, `AudioHighDRC`) and a suggested filename.
 ...
 *
 * @example
 * // 1. Running Basic Download Example (fetches highest quality audio, saves in current directory)
 * try {
 * const result = await AudioHighest({ query: "your search query or url" });
 ...
 * try {
 * const result = await AudioHighest({ query: "your search query or url", output: "./custom_downloads", filter: "bassboost" });
 ...
 * try {
 * const result = await AudioHighest({ query: "your search query or url", output: "./full_downloads", useTor: true, verbose: true, filter: "vaporwave", showProgress: true });
 ...
 * try {
 * const result = await AudioHighest({ query: "your search query or url", metadata: true });
 ...
 * try {
 * const result = await AudioHighest({ query: "your search query or url", metadata: true, useTor: true, verbose: true });
 ...
 *
 * @example
 * // 6. Running Basic Stream Example (fetches highest quality audio, pipes to a local file)
 * import { createWriteStream } from "fs";
 * try {
 * const result = await AudioHighest({ query: "your search query or url", stream: true });
 ...
 * import { createWriteStream } from "fs";
 * try {
 * const result = await AudioHighest({ query: "your search query or url", stream: true, filter: "nightcore" });
 * if ("stream" in result && result.stream) {
 ...
 * import { createWriteStream } from "fs";
 * try {
 * const result = await AudioHighest({ query: "your search query or url", stream: true, useTor: true, verbose: true, filter: "superspeed", showProgress: true });
 * if ("stream" in result && result.stream) {
 ...
 * try {
 * await AudioHighest({ query: "your search query or url", metadata: true, output: "./should_fail_dir" });
 ...
 * try {
 * await AudioHighest({ query: "your search query or url", stream: true, output: "./should_fail_dir" });
 * console.log("This should not be reached - Invalid Options Example.");
 * } catch (error) {
 ...
 * try {
 * await AudioHighest({} as any); // Using 'as any' to simulate missing required parameter
 ...
 * try {
 * await AudioHighest({ query: "your search query or url", filter: "nonexistentfilter" as any }); // Using 'as any' to simulate invalid enum value
 * console.log("This should not be reached - Zod Validation Error Example.");
 * } catch (error) {
 ...
 * // try {
 * //    await AudioHighest({ query: "query-that-breaks-tuber" });
 * // } catch (error) {
 ...
 * //    await AudioHighest({ query: "your search query or url" });
 * // } catch (error) {
 ...
 * //    await AudioHighest({ query: "some video with format issues" });
 * // } catch (error) {
 ...
 * //    await AudioHighest({ query: "query-causing-ffmpeg-error" });
 * // } catch (error) {
...
(async () => {
    ...
    try {
        ...
        await AudioHighest({ query: "your search query or url", filter: "nonexistentfilter" as any });
        console.log("This should not be reached - Zod Validation Error Example.");
    } catch (error) {
        console.error("Expected Zod Error (Invalid Filter):", error instanceof Error ? error.message : error);
    }
