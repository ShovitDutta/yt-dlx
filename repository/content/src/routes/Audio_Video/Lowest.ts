...
/**
 * @shortdesc Downloads or streams the lowest quality audio and video from YouTube.
 *
 * @description This function allows you to download or stream the lowest available audio and video qualities from YouTube based on a search query or video URL and merge them into a single file (Matroska, .mkv) or stream. It offers customization options such as saving the output to a specified directory, using Tor for anonymity, enabling verbose logging, streaming the output, or simply fetching the metadata without downloading. Video filters can also be applied.
 *
 * The function requires a search query or video URL. It automatically selects the lowest quality audio and video formats available and merges them.
 *
 * It supports the following configuration options:
 * - **query:** A string representing the search query or video URL. This is a mandatory parameter.
 * - **output:** An optional string specifying the directory where the output audio/video file should be saved. If not provided, the file will be saved in the current working directory. This parameter cannot be used when `metadata` is true.
 ...
 * - **stream:** An optional boolean value. If true, the audio/video will be streamed instead of saved to a file. When streaming, the `end` event will provide the streamable path and the `stream` event will provide the FFmpeg instance. This parameter cannot be used when `metadata` is true.
 ...
 * - **metadata:** An optional boolean value. If true, the function will only fetch and emit the video metadata and lowest quality stream details without downloading or streaming. When `metadata` is true, the `output`, `stream`, and `filter` parameters are not allowed.
 ...
 * - `"metadata"`: Emitted only when the `metadata` parameter is true. Provides an object containing the video metadata, lowest audio/video format details, and a suggested filename.
 ...
 *
 * @example
 * // 1. Download the lowest quality audio/video to the current directory
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url" })
 ...
 *
 * @example
 * // 2. Download the lowest quality audio/video to a custom output directory
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", output: "./lowest_av_downloads" })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("end", (outputPath) => console.log("Download finished:", outputPath))
 * .on("error", (error) => console.error("Error:", error));
 *
 * @example
 * // 3. Stream the lowest quality audio/video
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", stream: true })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("stream", (data) => {
 ...
 *
 * @example
 * // 4. Fetch only metadata for the lowest quality audio/video
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", metadata: true })
 * .on("metadata", (data) => console.log("Metadata:", data))
 * .on("error", (error) => console.error("Error:", error));
 * // Note: output, stream, and filter are ignored when metadata is true.
 *
 * @example
 * // 5. Download the lowest quality audio/video and apply a video filter
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", filter: "grayscale" })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("end", (outputPath) => console.log("Download finished:", outputPath))
 * .on("error", (error) => console.error("Error:", error));
 *
 * @example
 * // 6. Download the lowest quality audio/video and use Tor
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", useTor: true })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("end", (outputPath) => console.log("Download finished:", outputPath))
 * .on("error", (error) => console.error("Error:", error));
 *
 * @example
 * // 7. Download the lowest quality audio/video and enable verbose logging
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", verbose: true })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("end", (outputPath) => console.log("Download finished:", outputPath))
 * .on("error", (error) => console.error("Error:", error));
 *
 * @example
 * // 8. Download the lowest quality audio/video with custom output and apply a video filter
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", output: "./filtered_av", filter: "rotate180" })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("end", (outputPath) => console.log("Download finished:", outputPath))
 * .on("error", (error) => console.error("Error:", error));
 *
 * @example
 * // 9. Stream the lowest quality audio/video and apply a video filter
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", stream: true, filter: "flipHorizontal" })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("stream", (data) => {
 * console.log("Stream available:", data.filename);
 ...
 *
 * @example
 * // 10. Download the lowest quality audio/video with all applicable options (query, output, useTor, verbose, filter)
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", output: "./full_options_lowest", useTor: true, verbose: true, filter: "invert" })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("end", (outputPath) => console.log("Download finished:", outputPath))
 * .on("error", (error) => console.error("Error:", error));
 *
 * @example
 * // 11. Stream the lowest quality audio/video with all applicable options (query, stream, useTor, verbose, filter)
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", stream: true, useTor: true, verbose: true, filter: "rotate90" })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("stream", (data) => {
 * console.log("Stream available:", data.filename);
 ...
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", metadata: true, verbose: true, useTor: true })
 * .on("metadata", (data) => console.log("Metadata (Verbose, Tor):", data))
 * .on("error", (error) => console.error("Error:", error));
 *
 * @example
 * // 13. Attempt to use output with metadata (will result in an error)
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", metadata: true, output: "./should_fail" })
 ...
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", metadata: true, stream: true })
 ...
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", metadata: true, filter: "grayscale" })
 ...
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", stream: true, output: "./streamed_and_saved_av" })
 * .on("start", (start) => console.log("FFmpeg started:", start))
 * .on("progress", (progress) => console.log("Progress:", progress))
 * .on("stream", (data) => {
 * console.log("Stream event fired, file likely being saved to:", data.filename);
 * })
 * .on("end", (outputPath) => console.log("Process ended, file saved at:", outputPath)) // Should output the path in './streamed_and_saved_av'
 * .on("error", (error) => console.error("Error:", error));
 *
 * @example
 * // 17. Missing required 'query' parameter (will result in an error)
 * YouTubeDLX.Audio_Video.Lowest({} as any)
 ...
 * YouTubeDLX.Audio_Video.Lowest({ query: "your search query or url", filter: "nonexistentfilter" as any })
 ...
 * YouTubeDLX.Audio_Video.Lowest({ query: "a query that should return no results 12345abcde" })
