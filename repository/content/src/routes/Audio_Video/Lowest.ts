...
/**
 * @shortdesc Downloads or streams the lowest quality audio and video from YouTube videos/queries, with video filters.
 *
 * @description This function fetches the lowest available quality separate audio and video streams from a YouTube video identified by a query or URL.
 * It uses an internal engine (`Tuber`) to retrieve video information and the URLs for the lowest quality audio and video formats,
 * and then utilizes FFmpeg to combine these streams, process the video (optionally apply video filters), and output the result.
 * The output can be either a downloadable file saved locally or a readable stream.
 * The function also supports fetching only the video's metadata, including information about the lowest quality audio and video formats available.
 *
 ...
 * - **Filter:** An optional string specifying a video filter to apply using FFmpeg. Available filters include "invert", "rotate90", "rotate270", "grayscale", "rotate180", "flipVertical", "flipHorizontal". If no filter is specified, streams are copied without re-encoding video.
 * - **UseTor:** An optional boolean flag to route requests through Tor. Defaults to `false`.
 * - **Verbose:** An optional boolean value that, if true, enables detailed console logging during the fetch and processing stages, including FFmpeg commands. Defaults to `false`.
 * - **Metadata:** An optional boolean flag. If set to `true`, the function will only fetch and return the video metadata without performing any download or streaming. Cannot be used with `stream`, `output`, `filter`, or `showProgress`. Defaults to `false`. The returned metadata includes details about available lowest quality audio and video formats.
 ...
 * - If `metadata` is `true`, it returns a Promise resolving to `{ metadata: object }` containing detailed video information including data for the lowest quality audio and video formats (`BestAudioLow`, `AudioLowDRC`, `BestVideoLow`, `VideoLowHDR`, `ManifestLow`) and a suggested filename.
 * - If `stream` is `true` (and `metadata` is `false`), it returns a Promise resolving to `{ stream: Readable }` providing a stream of the combined audio and video data.
 ...
 *
 * FFmpeg is used internally to combine the lowest quality audio and video streams, apply video filters if specified, and output in 'matroska' (.mkv) format.
 *
 ...
 * - Throws an `Error` if the lowest quality video URL (`ManifestLow[0]?.url`) is not found in the fetched data.
 ...
 *
 * @example
 * // 1. Running Basic Download Example (fetches lowest audio+video, saves in current directory as MKV)
 * try {
 * const result = await AudioVideoLowest({ query: "your search query or url" });
 ...
 * try {
 * const result = await AudioVideoLowest({ query: "your search query or url", output: "./custom_downloads_avlow", filter: "grayscale" });
 ...
 * try {
 * const result = await AudioVideoLowest({ query: "your search query or url", output: "./full_downloads_avlow", useTor: false, verbose: true, filter: "invert", showProgress: true });
 ...
 * try {
 * const result = await AudioVideoLowest({ query: "your search query or url", metadata: true });
 ...
 * try {
 * const result = await AudioVideoLowest({ query: "your search query or url", metadata: true, useTor: false, verbose: true });
 ...
 *
 * @example
 * // 6. Running Basic Stream Example (fetches lowest audio+video, streams as MKV)
 * import { createWriteStream } from "fs";
 * try {
 * const result = await AudioVideoLowest({ query: "your search query or url", stream: true });
 * if ("stream" in result && result.stream) {
 * console.log("Basic Streaming started. Piping to basic_stream_avlow.mkv...");
 ...
 *
 * @example
 * // 7. Running Stream with Video Filter Example (Applies horizontal flip filter)
 * import { createWriteStream } from "fs";
 * try {
 * const result = await AudioVideoLowest({ query: "your search query or url", stream: true, filter: "flipHorizontal" });
 * if ("stream" in result && result.stream) {
 * console.log("Stream with Filter started. Piping to filtered_stream_avlow.mkv...");
 ...
 * if ("stream" in result && result.stream) {
 * console.log("\nStream with All Options started. Piping to full_stream_avlow.mkv...");
 ...
 * try {
 * await AudioVideoLowest({ query: "your search query or url", metadata: true, output: "./should_fail_dir" });
 ...
 * try {
 * await AudioVideoLowest({ query: "your search query or url", stream: true, output: "./should_fail_dir" });
 * console.log("This should not be reached - Invalid Options Example.");
 * } catch (error) {
 ...
 * try {
 * await AudioVideoLowest({} as any); // Using 'as any' to simulate missing required parameter
 ...
 * try {
 * await AudioVideoLowest({ query: "your search query or url", filter: "bassboost" as any }); // Using 'as any' to simulate invalid enum value
 * console.log("This should not be reached - Zod Validation Error Example.");
 * } catch (error) {
 ...
 * // try {
 * //    await AudioVideoLowest({ query: "query-that-breaks-tuber" });
 * // } catch (error) {
 ...
 * //    await AudioVideoLowest({ query: "your search query or url" });
 * // } catch (error) {
 * //    console.error("Expected Error (FFmpeg not found):", error instanceof Error ? error.message : error);
 * // }
 *
 * @example
 * // 15. Example of Lowest Quality Video URL not found
 * // This might occur for certain videos if the internal engine cannot find the expected format.
 * // try {
 * //    // Use a query for a video known to have issues with lowest quality video detection
 * //    await AudioVideoLowest({ query: "some video with low video issues" });
 * // } catch (error) {
 * //    console.error("Expected Error (Lowest Quality Video URL not found):", error instanceof Error ? error.message : error);
 * // }
 *
 * @example
 * // 16. Example of Lowest Quality Audio URL not found
 * // This might occur for certain videos if the internal engine cannot find the expected format.
 * // try {
 * //    // Use a query for a video known to have issues with lowest quality audio detection
 * //    await AudioVideoLowest({ query: "some video with low audio issues" });
 * // } catch (error) {
 ...
 * //    await AudioVideoLowest({ query: "query-causing-ffmpeg-error" });
 * // } catch (error) {
