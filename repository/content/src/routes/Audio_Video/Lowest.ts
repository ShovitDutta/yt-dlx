...
/**
 * @shortdesc Downloads or streams the lowest quality audio/video combined format from YouTube.
 *
 * @description This function fetches the lowest available quality combined audio and video streams from a YouTube video identified by a query or URL.
 * It uses an internal engine (`Tuber`) to retrieve video information and the URLs for the lowest quality streams,
 * and then utilizes FFmpeg to combine these streams, process them (optionally apply video filters), and output the result.
 ...
 * - **Stream:** An optional boolean flag. If set to `true`, the function returns a Node.js Readable stream of the video/audio data instead of saving a file. Cannot be used with `output`. Defaults to `false`.
 ...
 * - If `metadata` is `true`, it returns a Promise resolving to `{ metadata: object }` containing detailed video information, including data about the lowest quality audio (`BestAudioLow`, `AudioLowDRC`) and video (`BestVideoLow`, `VideoLowHDR`, `ManifestLow`) formats, and a suggested filename.
 * - If `stream` is `true` (and `metadata` is `false`), it returns a Promise resolving to `{ stream: Readable }` providing a stream of the video/audio data.
 ...
 *
 * FFmpeg is used internally to combine the separate lowest quality video and audio streams and output them as a Matroska (`.mkv`) file.
 *
 * @param {object} options - The configuration options for the video/audio fetching and processing.
 ...
 * - Throws an `Error` if the lowest quality video URL (`ManifestLow[0]?.url`) or lowest quality audio URL (`BestAudioLow?.url`) is not found in the fetched data.
 ...
 *
 * @example
 * // 1. Running Basic Download Example (fetches lowest quality audio/video, saves in current directory)
 * try {
 ...
 *
 * @example
 * // 2. Running Download with Output and Filter Example (applies grayscale filter)
 * try {
 * const result = await AudioVideoLowest({ query: "your search query or url", output: "./custom_downloads", filter: "grayscale" });
 ...
 * try {
 * const result = await AudioVideoLowest({ query: "your search query or url", output: "./full_downloads", useTor: true, verbose: true, filter: "invert", showProgress: true });
 ...
 * try {
 * const result = await AudioVideoLowest({ query: "your search query or url", metadata: true, useTor: true, verbose: true });
 ...
 *
 * @example
 * // 6. Running Basic Stream Example (fetches lowest quality, pipes to a local file)
 ...
 *
 * @example
 * // 7. Running Stream with Filter Example (applies horizontal flip, pipes to a local file)
 * import { createWriteStream } from "fs";
 * try {
 ...
 *
 * @example
 * // 8. Running Stream with All Options Example (applies rotate90 filter, pipes to a local file)
 * import { createWriteStream } from "fs";
 * try {
 * const result = await AudioVideoLowest({ query: "your search query or url", stream: true, useTor: true, verbose: true, filter: "rotate90", showProgress: true });
 * if ("stream" in result && result.stream) {
 ...
 * try {
 * await AudioVideoLowest({ query: "your search query or url", filter: "nonexistentfilter" as any }); // Using 'as any' to simulate invalid enum value
 * console.log("This should not be reached - Zod Validation Error Example.");
 * } catch (error) {
 ...
 *
 * @example
 * // 15. Example of Lowest Quality Stream URL(s) not found
 * // This might occur for certain videos if the internal engine cannot find the expected formats.
 * // try {
 * //    // Use a query for a video known to have issues with lowest quality audio/video detection
 * //    await AudioVideoLowest({ query: "some video with format issues" });
 * // } catch (error) {
 * //    console.error("Expected Error (Lowest Quality Stream URL(s) not found):", error instanceof Error ? error.message : error);
 * // }
 *
 * @example
...
(async () => {
    ...
    try {
        ...
        await AudioVideoLowest({ query: "your search query or url", filter: "nonexistentfilter" as any });
        console.log("This should not be reached - Zod Validation Error Example.");
    } catch (error) {
        console.error("Expected Zod Error (Invalid Filter):", error instanceof Error ? error.message : error);
    }
