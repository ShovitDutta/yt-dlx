...
/**
 * @shortdesc Fetches the highest quality audio and combines it with the video thumbnail, applying video filters.
 *
 * @description This function retrieves the highest available quality audio and the video thumbnail from a YouTube video identified by a query or URL.
 * It uses an internal engine (`Tuber`) to get the relevant URLs and metadata,
 * and then utilizes FFmpeg to combine the audio stream with the thumbnail image, optionally applying video filters to the thumbnail stream, and outputting to an AVI file or a stream.
 * The function also supports fetching only the video's metadata.
 *
 * **Note:** As implemented, this function combines the highest quality audio stream with the video's thumbnail image and applies video filters to the thumbnail stream, outputting an AVI file. It does *not* combine the highest quality *video stream* from the YouTube source with the highest quality audio stream.
 *
 ...
 * - **Output:** An optional string specifying the directory path where the downloaded AVI file should be saved. If not provided, the file is saved in the current working directory. Cannot be used with `stream`.
 * - **Stream:** An optional boolean flag. If set to `true`, the function returns a Node.js Readable stream of the AVI data instead of saving a file. Cannot be used with `output`. Defaults to `false`.
 * - **Filter:** An optional string specifying a *video* filter to apply using FFmpeg. Available filters include "invert", "rotate90", "rotate180", "rotate270", "grayscale", "flipVertical", "flipHorizontal". These filters are applied to the thumbnail image stream.
 ...
 * - If `metadata` is `true`, it returns a Promise resolving to `{ metadata: object }` containing detailed video information including audio and video format data (`AudioHighF`, `AudioHighDRC`, `VideoHighF`, `VideoHighHDR`, `ManifestHigh`) and a suggested filename.
 * - If `stream` is `true` (and `metadata` is `false`), it returns a Promise resolving to `{ stream: Readable }` providing a stream of the AVI data.
 * - Otherwise (if `metadata` is `false` and `stream` is `false`), it returns a Promise resolving to `{ outputPath: string }` indicating the path to the downloaded AVI file.
 *
 * FFmpeg is used internally, taking the highest audio stream and the video thumbnail as inputs, applying the specified video filter to the thumbnail (if any), and outputting to the AVI format. If no filter is applied, streams are copied.
 *
 ...
 * @param {"invert" | "rotate90" | "rotate180" | "rotate270" | "grayscale" | "flipVertical" | "flipHorizontal"} [options.filter] - Apply a video filter to the thumbnail stream.
 *
 ...
 *
 * @example
 * // 1. Running Basic Download Example (fetches highest audio and thumbnail, saves in current directory as AVI)
 * try {
 ...
 *
 * @example
 * // 2. Running Download with Output and Video Filter Example (applies grayscale to thumbnail)
 * try {
 * const result = await AudioVideoHighest({ query: "your search query or url", output: "./custom_downloads", filter: "grayscale" });
 ...
 * try {
 * const result = await AudioVideoHighest({ query: "your search query or url", output: "./full_downloads", useTor: true, verbose: true, filter: "flipHorizontal", showProgress: true });
 ...
 * try {
 * const result = await AudioVideoHighest({ query: "your search query or url", metadata: true, useTor: true, verbose: true });
 ...
 *
 * @example
 * // 6. Running Basic Stream Example (fetches highest audio and thumbnail, pipes AVI to a local file)
 ...
 *
 * @example
 * // 7. Running Stream with Video Filter Example (applies rotate90 to thumbnail)
 * import { createWriteStream } from "fs";
 * try {
 ...
 * import { createWriteStream } from "fs";
 * try {
 * const result = await AudioVideoHighest({ query: "your search query or url", stream: true, useTor: true, verbose: true, filter: "rotate270", showProgress: true });
 * if ("stream" in result && result.stream) {
 ...
 * //    await AudioVideoHighest({ query: "some video with audio format issues" });
 * // } catch (error) {
 ...
 * // This might occur for certain videos if the internal engine cannot find a thumbnail.
 * // try {
 * //    // Use a query for a video known to have missing thumbnails
 * //    await AudioVideoHighest({ query: "some video with no thumbnail" });
 * // } catch (error) {
...
(async () => {
    ...
    try {
        ...
        await AudioVideoHighest({} as any);
        console.log("This should not be reached - Zod Validation Error Example.");
    } catch (error) {
        console.error("Expected Zod Error (Missing Query):", error instanceof Error ? error.message : error);
    }
    ...
    try {
        ...
        await AudioVideoHighest({ query: "your search query or url", filter: "bassboost" as any });
        console.log("This should not be reached - Zod Validation Error Example.");
    } catch (error) {
        console.error("Expected Zod Error (Invalid Filter):", error instanceof Error ? error.message : error);
    }
