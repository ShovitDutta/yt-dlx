...
describe("AudioVideoHighest", () => {
    ...
    it("should handle download with output and filter", async () => {
        const result = await AudioVideoHighest({ query, output: "./custom_downloads_avhigh", filter: "grayscale" });
        expect(result).toHaveProperty("outputPath");
        if ("outputPath" in result) {
            expect(result.outputPath).toMatch(/\.avi$/);
        }
    ...
    it("should handle download with all options", async () => {
        const result = await AudioVideoHighest({ query, output: "./full_downloads_avhigh", useTor: false, verbose: true, filter: "flipHorizontal", showProgress: true });
        expect(result).toHaveProperty("outputPath");
        if ("outputPath" in result) {
            expect(result.outputPath).toMatch(/\.avi$/);
        }
    ...
    it("should throw error for metadata with output", async () => {
        await expect(AudioVideoHighest({ query: "test query", metadata: true, output: "./should_fail_dir" })).rejects.toThrowError(/metadata.*cannot be used with.*output/);
    });
    it("should throw error for stream with output", async () => {
        await expect(AudioVideoHighest({ query: "test query", stream: true, output: "./should_fail_dir" })).rejects.toThrowError(/stream.*cannot be used with.*output/);
    });
    it("should throw Zod error for missing query", async () => {
        await expect(AudioVideoHighest({} as any)).rejects.toThrowError(/query.*Required/);
    });
    it("should throw Zod error for invalid filter", async () => {
        await expect(AudioVideoHighest({ query: "test query", filter: "bassboost" as any })).rejects.toThrowError(/filter.*invalid enum value/);
    });
});
