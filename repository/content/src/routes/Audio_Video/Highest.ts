...
// Define the Zod schema for input validation for AudioHighest with video filter names
var ZodSchema = z.object({
    ...
    filter: z // Filter enum with video filter names as provided in the original code
...
type MetadataResult = {
    ...
    AudioHighF: any; // Details for the highest quality audio format
    AudioHighDRC: any; // Details for highest audio with DRC
    VideoHighF: any; // Details for highest quality video format
...
/**
 * @shortdesc Downloads, streams, or fetches metadata for the highest quality audio from YouTube with filter options using async/await.
 *
 * @description This function allows you to download or stream the highest available audio quality from YouTube based on a search query or video URL using async/await.
 * It offers customization options such as saving the output to a specified directory, using Tor for anonymity, enabling verbose logging, streaming the output, or simply fetching the metadata. An optional filter parameter is accepted.
 *
 * The function requires a search query or video URL. It automatically selects the highest quality audio format available. It supports providing *video* filter names via the `filter` option, though these filters are applied using FFmpeg's `withVideoFilter` which is designed for video streams and may not affect the output audio.
 *
 ...
 * @param {("invert" | "rotate90" | "rotate270" | "grayscale" | "rotate180" | "flipVertical" | "flipHorizontal")} [options.filter] - A video filter to potentially apply. Note: These filters are applied using FFmpeg's video filter options (`withVideoFilter`) to an audio-only input stream and are unlikely to affect the audio output. Ignored when `metadata` is true.
 *
 * @returns {Promise<AudioHighestResult>} A Promise that resolves based on the operation mode:
 * - If `metadata` is true: Resolves with a `MetadataResult` object containing metadata including format details.
    ...
    try {
        ...
        if (metadata) {
            ...
            return {
                ...
                AudioHighF: engineData.AudioHighF, // Include highest audio format details from engineData
                AudioHighDRC: engineData.AudioHighDRC, // Include highest audio with DRC details from engineData
                VideoHighF: engineData.VideoHighF, // Include highest video format details from engineData
                VideoHighHDR: engineData.VideoHighHDR, // Include highest HDR video details from engineData
        ...
        // Select the highest quality audio format found by the engine based on 'AudioHighF'.
        const hqAudio = engineData.AudioHighF;

        // Check if the highest quality audio format object exists and has a URL.
        if (!hqAudio?.url) {
        ...
        // Add the URL of the highest quality audio stream as the input to FFmpeg.
        ...
        // Note: In audio-only output, the thumbnail input is typically used for album art.
        instance.addInput(engineData.metaData.thumbnail);

        // Set the output format for FFmpeg. Original used 'avi', which is primarily a video container.
        // Using 'mp3' or 'm4a' would be more typical for audio, but preserving original format for refactoring.
        ...
        // Use .avi extension as in the original code.
        ...
        // Apply the specified filter if provided and valid.
        // Note: The original code applied these *video* filters using `withVideoFilter`.
        // Since only an audio stream is added as input, applying a video filter is unlikely to have an effect
        // on the output audio and might cause FFmpeg warnings or errors depending on the version and flags.
        // Preserving this original logic as requested ("do same"), but highlighting the potential mismatch.
        if (filter && filterMap[filter]) {
            // Apply the video filter complex options using `withVideoFilter`.
            // This is likely functionally incorrect for audio-only input but matches original code.
            instance.withVideoFilter(filterMap[filter]);
            if (verbose)
                console.log(colors.yellow("@warning:"), `Applied video filter '${filter}' using withVideoFilter() to an audio-only input stream. This may not have an effect on the output audio.`);
        } else {
