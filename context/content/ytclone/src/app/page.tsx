...
const HomeFeed = ({ homeFeed, isLoading, onGetHomeFeed }: { homeFeed: VideoType[]; isLoading: boolean; onGetHomeFeed: () => void }) => {
    return isLoading ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1, duration: 0.5 }}>
            ...
            <button className="bg-green-500 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-md focus:outline-none focus:shadow-outline mb-4" onClick={onGetHomeFeed}>
                Get Home Feed
            </button>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                {homeFeed.map(video => (
