    ...
    async run(): Promise<void> {
        ...
        try {
            ...
            await new Promise<void>(async (resolve, reject) => {
                ...
                if (this.options.Verbose) console.log(colors.green("@info: ") + "Running FFmpeg Command...");
                instance.on("end", () => resolve());
                instance.on("error", err => reject(err));
            });
            if (isAudioM3u8 && (await fs.pathExists(segmentsDir))) {
                if (this.options.Verbose) console.log(colors.green("@info: ") + "Removing Segments Directory...");
