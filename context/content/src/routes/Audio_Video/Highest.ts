...
/**
 * @shortdesc Downloads or streams a YouTube video with the highest available audio and video quality, with optional filtering and metadata retrieval.
 *
 * @description This function retrieves the highest available quality video stream and the highest available quality audio stream (by default or specified language) from a given YouTube query (URL or video ID).
 ...
 * @param options.MetaData - An optional boolean. If `true`, only the metadata (title, file name, and highest quality video/audio links) of the content will be returned, without downloading or streaming. This parameter cannot be used with `Stream`, `Output`, `Filter`, or `ShowProgress`.
 ...
 * - If `MetaData` is `true`: An object containing `MetaData` (detailed video metadata), `FileName` (suggested file name), and `Links` (highest quality standard/HDR video formats and highest quality standard/DRC audio formats available by language).
 ...
 * - If the highest quality video URL is not found: `Error: @error: Highest quality video URL not found.`
 * - If the highest quality audio URL is not found for the specified (or default) language: `Error: @error: Highest quality audio URL not found for language: [language_or_default].`
 ...
 */
export default async function Audio_Video_Highest({ Query, Output, UseTor, Stream, Filter, MetaData, Verbose, ShowProgress, AudioLanguage }: AudioVideoHighestOptions): Promise<
    | {
            ...
            return {
                ...
                Links: {
                    Audio: {
                        ...
                        DRC_Highest: Object.fromEntries(Object.entries(EngineMeta.AudioOnly.Dynamic_Range_Compression).map(([lang, data]) => [lang, data?.Highest as CleanedAudioFormat | null])),
                    },
                    Video: {
                        Standard_Lowest: EngineMeta.VideoOnly.Standard_Dynamic_Range.Highest as CleanedVideoFormat | null,
                        HDR_Lowest: EngineMeta.VideoOnly.High_Dynamic_Range.Highest as CleanedVideoFormat | null,
                    },
                },
            };
        }
