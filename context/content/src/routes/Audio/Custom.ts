...
export default async function AudioCustom({
    ...
    try {
        ZodSchema.parse({ Query, Output, UseTor, Stream, Filter, Verbose, MetaData, Resolution, ShowProgress, Language });
        ...
        const targetLanguage = Language || "Unknown";
        let AudioMeta: CleanedAudioFormat | undefined;
        if (EngineMeta.AudioOnly.Standard[targetLanguage]?.Combined) {
            AudioMeta = EngineMeta.AudioOnly.Standard[targetLanguage].Combined.find(i => i.format_note?.toLowerCase().includes(Resolution.toLowerCase()));
        }
        if (!AudioMeta && EngineMeta.AudioOnly.Dynamic_Range_Compression[targetLanguage]?.Combined) {
            AudioMeta = EngineMeta.AudioOnly.Dynamic_Range_Compression[targetLanguage].Combined.find(i => i.format_note?.toLowerCase().includes(Resolution.toLowerCase()));
        }
        if (!AudioMeta) {
            throw new Error(
                `${colors.red(
        ...
        if (!AudioMeta.url) throw new Error(`${colors.red("@error:")} The audio URL was not found for Resolution "${Resolution}" and language "${targetLanguage}".`);
