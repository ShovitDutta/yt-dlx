        ...
        socket.on("yt-dlx-command", async data => {
            ...
            if (import_yt_dlx.default[command]) {
                ...
                if (typeof ytDlxFunction === "function") {
                    try {
                        const resultEmitter = ytDlxFunction(options)
                            .on("data", resultData => socket.emit(`${command}-response`, { status: "success", data: resultData }))
                            .on("metadata", resultData => socket.emit(`${command}-response`, { status: "success", data: resultData }))
                            .on("error", error => socket.emit(`${command}-response`, { status: "error", message: error.message || error }));
